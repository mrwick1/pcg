...
async function populateRouteDropdownOptions() {
    ...
    try {
        ...
        if (typeof fetchProjectsData !== 'function' || typeof fetchResourcesData !== 'function' || typeof fetchBillingData !== 'function') {
            console.log('New API functions not available yet, will retry...');
            setTimeout(populateRouteDropdownOptions, 100);
            return;
        }
        
        // Fetch from three separate APIs
        ...
        console.log('Fetched billing count:', billing.length);
        ...
        projects.forEach(location => {
            if (location && location.projectStructure && location.projectStructure.projectName && location.lat && location.lng) {
                allOptions.push({
                    id: location.projectStructure.pccNumber,
                    name: location.projectStructure.projectName,
                    lat: location.lat,
                    lng: location.lng,
                    type: 'project',
                    address: location.projectStructure.projectAddress
                });
            }
        });
